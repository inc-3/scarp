...
\x1b[38;5;46m└──────────────────────────────────────────────────────┘ 
{}
\x1b[38;5;46         WELLCOME RJ FILE DUMPING-PRO TOOL
{}'''.format('\n',50*'-',50*'-')
os.system('xdg-open ')
...
class login():
	...
	def lo_epa(self):
		...
		if 'access_token' in po:
			...
			exit('run again python RJ-FILE.py')
		else:
	...
	def login_epa2(self):
		...
		if 'access_token' in req:
			...
			exit('\033[1;31m Invalid Cookie Something Wrong')
	def login_at(self):
		system('clear');
		print(logo)
		...
		try:
			headers = {"X-Graphql-Client-Library": "graphservice", "X-Graphql-Request-Purpose": "fetch",
					   ...
					   "X-Fb-Net-Hni": "61001", "X-Fb-Sim-Hni": "41001",
			...
			try:
				...
				print(e)
				exit(f' \033[1;31invalid or expire token {uid}\033[0m ')
		except Exception as e:
			os.system('clear');
			print(logo)
	...
	def login_WALA(self):
		system('clear');print(logo)
		print(" IF YOU SHOW ANY PROBLEM POST RJ COMMAND WORLD GROUP ")
		print(50 * '-')
		print('(01) Login With Email Pass ')
...
def main_menu():
	...
	if menu in ['11','011']:
		os.system("xdg-open ")
	if menu in ['00','0']:
		exit('Thanks For Using RJ FILE DUMP-PRO TOOL  !')
	else:
		exit(' invalid ')
siid=[]
sep=[]
class file_create_menu():
	...
	def file_unlimmited(self):
		...
		try:
			...
			file = []
		os.system('clear');print(logo)
		print(' Total ids File is : ' + str(len(file)))
		print(' YOUR FILE MAKING STARTED JUST WAIT')
		print(50*'-')
		for uid in file:
			try:
				headers = {"X-Graphql-Client-Library": "graphservice", "X-Graphql-Request-Purpose": "fetch",
	...
	def new_ids(self):
		...
		while True:
			ids_all = input("")
			if ids_all == "":
				exit('sucessfully your file make')
				break
			try:
				uid = ids_all.split("|")[0]
			except:
